...
export function renderComponent(node: ComponentNode, ctx: RendererContext, bundle: CodeBundle | null, theme?: UITheme): ReactNode {
  ...
  switch (node.type) {
    ...
    case "navbar": {
      const navLinks = links(node.props.links);
      return <nav key={node.id} className={`mx-auto flex w-full flex-wrap items-center justify-between gap-3 border-b px-5 py-4 md:px-6 ${extra}`} style={{ ...panel, ...shell }}><button type="button" className="text-left text-lg font-semibold transition-opacity hover:opacity-90" style={text} onClick={() => ctx.navigate(navLinks[0]?.route ?? "/")}>{String(node.props.title ?? "Application")}</button><div className="flex flex-wrap items-center gap-2">{navLinks.map((link) => { const active = ctx.currentPage === link.route || (link.route !== "/" && ctx.currentPage.startsWith(link.route)); return <button key={`${node.id}-${link.label}-${link.route}`} type="button" onClick={() => ctx.navigate(link.route)} className="rounded-full border px-3 py-1.5 text-sm font-medium transition-opacity hover:opacity-90" style={active ? { borderColor: "var(--na-border)", backgroundColor: "var(--na-accent-soft)", color: "var(--na-primary)" } : secondary}>{link.label}</button>; })}</div></nav>;
    }
